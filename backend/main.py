...
pool = None
cache_dir = os.path.join(tempfile.gettempdir(), "ymkw_api_diskcache_v7")
...
def format_ranking_response(rows):
    return [
        {"user_id": str(r["user_id"]), "display_name": r["display_name"] or "Unknown",
         "username": r["username"] or "unknown", "avatar": r["avatar_url"],
         "count": r["c"], "char_count": r["chars"] or 0}
        for r in rows
    ]
...
async def get_daily_history(year: int, month: int, response: Response, channel_id: Optional[int] = Query(None), user_id: Optional[str] = Query(None)):
    ckey = f"hist_m_{year}_{month}_{channel_id}_{user_id}"
    cached = get_cache(ckey)
    response.headers["Cache-Control"] = "public, max-age=600"
    if cached: return cached
    c_f = "AND channel_id = $3" if channel_id else ""
    t_rows = await pool.fetch(f"SELECT DATE(created_at) as d, count(*) as c FROM messages WHERE extract(year from created_at) = $1 AND extract(month from created_at) = $2 AND is_bot = FALSE {c_f} GROUP BY DATE(created_at) ORDER BY d", year, month, *([channel_id] if channel_id else []))
    top_u = await pool.fetch(f"SELECT m.user_id, count(*) as c FROM messages m LEFT JOIN users u ON m.user_id = u.user_id WHERE extract(year from m.created_at) = $1 AND extract(month from m.created_at) = $2 AND m.is_bot = FALSE AND {DELETED_USER_FILTER} {c_f.replace('channel_id', 'm.channel_id')} GROUP BY m.user_id ORDER BY c DESC LIMIT 100", year, month, *([channel_id] if channel_id else []))
    target_ids = [str(r['user_id']) for r in top_u]
    if user_id and user_id.isdigit() and user_id not in target_ids: target_ids.append(user_id)
    ...
    if target_ids:
        ...
        p_idx = f"${len(params)+1}"
        rows = await pool.fetch(f"SELECT DATE(created_at) as d, user_id, count(*) as c FROM messages WHERE extract(year from created_at) = $1 AND extract(month from created_at) = $2 AND is_bot = FALSE {c_f} AND user_id = ANY({p_idx}::bigint[]) GROUP BY DATE(created_at), user_id ORDER BY d", *(params + [ids_plist]))
        for r in rows:
...
async def get_total_history(response: Response, channel_id: Optional[int] = Query(None), user_id: Optional[str] = Query(None), end_date: Optional[datetime] = Query(None)):
